...
const ROUTE_WITHOUT_NAVBAR = ['/login', '/personal-finance']

function Index() {
  const {user} = useRootData()
  const location = useLocation()
  const result = ROUTE_WITHOUT_NAVBAR.findIndex(loc => {
    return loc.startsWith(location.pathname)
  }, location.pathname)
  const isShowNavbar =
    ROUTE_WITHOUT_NAVBAR.includes(location.pathname) || result
