...
const USER_LINKS = [
  {name: 'Cashflow', to: '/cash-flow', Icon: <Wallet2 size={18} />},
]
const OWNER_LINKS = [
      ...
      {user ? (
        ...
        <li className="px-4 py-2">
          <Link to="/login" prefetch="intent">
            <button
              type="button"
              className="text-secondary block w-fit whitespace-nowrap rounded-lg border border-gray-600 px-3 pb-1.5 pt-1 text-md font-medium hover:border-white hover:bg-gray-800 hover:text-white"
            >
              Log in
            </button>
          </Link>
...
function ProtectedNavItems({
  ...
  if (!user) return <></>
  return (
    <div className="sticky top-0 z-50 w-full border-b border-b-gray-600 bg-black px-[4vw] py-[19px] xl:px-10vw">
...
function ProtectedpNavLink({
  ...
  return (
    <ButtonLink
      ...
      variant="subtle"
      to={to}
      className={clsx('flex items-center gap-x-2 hover:bg-gray-800', {
