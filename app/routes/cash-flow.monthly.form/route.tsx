...
import { createPost, deletePost, getPost, getPostByAuthor, updatePost } from "~/utils/post.session"
...
function BackButton() {
  return (
    <Link to="/cash-flow/monthly" prefetch="intent">
      <UIButton type="button" variant="subtle" className="text-md text-orange-500">
...
function FormEditor({ submitRef, type }: { submitRef: React.RefObject<HTMLInputElement>, type: FormType }) {
  ...
  return (
    ...
    >
      ...
      <input ref={postJSONRef} defaultValue={JSON.stringify(post?.content)} type="hidden" name="postJSON" />
      <input
        readOnly
        type="hidden"
        name="postId"
